...
Java 8 interface changes include static methods and default methods in interfaces. Prior to Java 8, we could have only
method declarations in the interfaces. But from Java 8, we can have default methods and static methods in the
interfaces.

**Default methods** enable you to add new functionality to existing interfaces and ensure binary compatibility with code
...
A functional interface in Java is an interface that contains only one abstract method. They are also known as Single
Abstract Method (SAM) interfaces. Functional interfaces are a key feature introduced in Java 8 to support lambda
expressions and functional programming constructs. While functional interfaces can have multiple default or static
methods, they must have exactly one abstract method to qualify as a functional interface.
...
* Function: Takes 1 input, returns result
* Consumer: Takes 1 input, returns nothing
* Supplier: Takes no input, returns result

Functional interfaces provide a foundation for functional programming constructs in Java, allowing developers to express
behavior as data and write more expressive and concise code using lambda expressions. They are widely used in
conjunction with streams, collections, and other APIs introduced in Java 8 and later versions.

---

### Difference between Collection and Stream

Collections store and manage groups of objects or elements. They represent data structures that hold multiple elements
as a single unit. Most collections in Java are mutable, meaning that their contents can be modified after they are
created. Elements can be added, removed, or modified within a collection. Collections typically provide direct access to
their elements via indexes (e.g., List), allowing for random access and manipulation of elements by their position in
the collection.

Streams represent a sequence of elements that support aggregate operations. They provide a way to process data in a
declarative and functional style. Streams are immutable data structures. Once created, a stream cannot be modified.
Instead, stream operations return new streams with the desired transformations. Streams support lazy evaluation, meaning
that elements are processed on-demand as they flow through a pipeline of intermediate and terminal operations.

---

### What is Terminal Operator vs Intermediate operators?

Intermediate Operation always returns a stream and can be chained together. A pipeline can have any number of
intermediate operations. They don't produce any result. eg: filter(), map(), distinct(), skip(), limit();

Terminal operation returns a result and hence, can't be chained together. A pipeline can have only 1 terminal operation.
eg: forEach(), reduce(), findFirst(), anyMatch(), toList(), collect() etc.

---

### What is Optional?

Optional is a new class in Java 8 that encapsulates an optional value, i.e. a value that is either there or not. It’s a
wrapper around an object. Optional has a special Optional.empty() value instead of wrapped null. Thus, it can be used
instead of a nullable value to get rid of NullPointerException in many cases.

---

### Flatmap vs Map?

In Java, the Stream interface has a map() and flatmap() methods and both have intermediate stream operation and return
another stream as method output. Both of the functions map() and flatMap are used for transformation and mapping
operations. map() function produces one output for one input value, whereas flatMap() function produces an arbitrary no
of values as output (ie zero or more than zero) for each input value.

Eg. Consider a list: [Apple, mango, pineapple, kiwi]

List generated by map-[5, 5, 9, 4] (Length of strings)

List of list-[[1, 2], [3, 4], [5, 6], [7, 8]]

List generate by flatMap-[1, 2, 3, 4, 5, 6, 7, 8]

[Full Code here](https://www.geeksforgeeks.org/difference-between-map-and-flatmap-in-java-stream/)

---

### Difference between Parallel sort vs sort

The sort() method sorts the array of objects or primitives. The sorting algorithm used in this method is
Dual-Pivot Quicksort. In other words, it is a custom implementation of the Quicksort algorithm to achieve better
performance.

The parallelSort() is functionally different. Unlike sort(), which sorts data sequentially using a single thread, it
uses a parallel sort-merge sorting algorithm. It breaks the array into sub-arrays that are themselves sorted and then
merged.
...
Predicate: Represents a predicate of one argument. Takes a single input argument and returns a boolean result based on
some condition.

BiPredicate: Represents a predicate of two arguments. Takes two input arguments and returns a boolean result based on
some condition involving both arguments.

---

### How Diamond problem is solved in Java 8?

In Java 8, the diamond problem is addressed through the introduction of default methods in interfaces. The diamond
problem occurs when a class inherits from multiple superclasses that have a common ancestor, and those superclasses
provide conflicting implementations of the same method.

Prior to Java 8, if a class implemented multiple interfaces with conflicting default method implementations, the
compiler would raise an error, requiring the implementing class to provide its own implementation of the conflicting
method.

Java 8 introduced the concept of default methods in interfaces, which allows interfaces to provide default
implementations for methods. This feature provides a solution to the diamond problem by allowing interfaces to inherit
multiple default methods with the same signature, and the implementing class can choose which default method to use or
override.
